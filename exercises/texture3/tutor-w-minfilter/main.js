...
var theTexture = nascar;        // initial value. callbacks modify this
...
function displayPlane (texture) {
    ...
    texture.minFilter = minOptions[params.minFilter];
    ...
    scene.add(plane);
    plane.lookAt(new THREE.Vector3(0,1,1));
}
...
gui.add(params, 'image', Object.keys(images))
    .onChange(val => { theTexture = images[val];
                       displayPlane(theTexture); });
gui.add(params, 'minFilter', Object.keys(minOptions)).onChange(() => displayPlane(theTexture));
